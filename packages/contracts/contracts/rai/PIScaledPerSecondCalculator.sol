...
contract PIScaledPerSecondCalculator {
    // --- Authorities ---
    ...
    constructor(
        ...
        require(both(feedbackOutputUpperBound_ < (type(uint256).max - defaultRedemptionRate) - 1, feedbackOutputUpperBound_ > 0), "PIScaledPerSecondCalculator/invalid-foub");
        ...
        require(uint(importedState[0]) <= block.timestamp, "PIScaledPerSecondCalculator/invalid-imported-time");
    ...
    */
    function modifyParameters(bytes32 parameter, uint256 val) external isAuthority {
        if (parameter == "nb") {
          ...
          require(both(val < ((type(uint256).max - defaultRedemptionRate) - 1), val > 0), "PIScaledPerSecondCalculator/invalid-foub");
    ...
    function getBoundedRedemptionRate(int piOutput) public isReader view returns (uint256, uint256) {
        ...
        if (negativeOutputExceedsHundred) {
          ...
          if (boundedPIOutput < 0 && boundedPIOutput <= -int(NEGATIVE_RATE_LIMIT)) {
            newRedemptionRate = uint(int(defaultRedemptionRate) + -int(NEGATIVE_RATE_LIMIT));
          } else {
            // Otherwise add defaultRedemptionRate and boundedPIOutput together
            newRedemptionRate = uint(int(defaultRedemptionRate) + boundedPIOutput);
          }
        }
    ...
    */
    function breaksNoiseBarrier(uint piSum, uint redemptionPrice) public isReader view returns (bool) {
        uint deltaNoise = (uint(2) * EIGHTEEN_DECIMAL_NUMBER) - noiseBarrier;
        return piSum >= (((redemptionPrice * deltaNoise) / EIGHTEEN_DECIMAL_NUMBER) - redemptionPrice);
    }
    /*
    ...
    */
    function getNextPriceDeviationCumulative(int proportionalTerm, uint accumulatedLeak) public isReader view returns (int256, int256) {
        ...
        int256 newTimeAdjustedDeviation  = (riemannSum(proportionalTerm, lastProportionalTerm) * int(timeElapsed));
        int256 leakedPriceCumulative     = ((int(accumulatedLeak) * priceDeviationCumulative) / int(TWENTY_SEVEN_DECIMAL_NUMBER));

        return (
          (leakedPriceCumulative + newTimeAdjustedDeviation),
    ...
    */
    function getGainAdjustedTerms(int proportionalTerm, int integralTerm) public isReader view returns (int256, int256) {
        return (
          (proportionalTerm * int(controllerGains.Kp)) / int(EIGHTEEN_DECIMAL_NUMBER),
          (integralTerm * int(controllerGains.Ki)) / int(EIGHTEEN_DECIMAL_NUMBER)
        );
    }
    ...
    */
    function computeRate(
      ...
      uint accumulatedLeak
        ...
        require((block.timestamp - lastUpdateTime) >= integralPeriodSize || lastUpdateTime == 0, "PIScaledPerSecondCalculator/wait-more");
        // Scale the market price by 10^9 so it also has 27 decimals like the redemption price
        uint256 scaledMarketPrice = (marketPrice * 10**9);
        // Calculate the proportional term as (redemptionPrice - marketPrice) * TWENTY_SEVEN_DECIMAL_NUMBER / redemptionPrice
        int256 proportionalTerm = ((int(redemptionPrice) - int(scaledMarketPrice)) * int(TWENTY_SEVEN_DECIMAL_NUMBER)) / int(redemptionPrice);
    ...
    */
    function tlv() external isReader view returns (uint256) {
        uint elapsed = (lastUpdateTime == 0) ? 0 : (block.timestamp - lastUpdateTime);
        return elapsed;
    }
}
