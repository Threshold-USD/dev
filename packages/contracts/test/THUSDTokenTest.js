  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    if (!withProxy) {
      it('Initial set of contracts was set correctly', async () => {
        const troveManagerRoles = await thusdTokenTester.getRoles(troveManager.address)
        assert.equal(troveManagerRoles[0], false)
        assert.equal(troveManagerRoles[1], true)
        assert.equal(troveManagerRoles[2], false)
        assert.equal(troveManagerRoles[3], true)
        
        const stabilityPoolRoles = await thusdTokenTester.getRoles(stabilityPool.address)
        assert.equal(stabilityPoolRoles[0], false)
        assert.equal(stabilityPoolRoles[1], true)
        assert.equal(stabilityPoolRoles[2], true)
        assert.equal(stabilityPoolRoles[3], true)
        
        const borrowerOperationsRoles = await thusdTokenTester.getRoles(borrowerOperations.address)
        assert.equal(borrowerOperationsRoles[0], true)
        assert.equal(borrowerOperationsRoles[1], true)
        assert.equal(borrowerOperationsRoles[2], false)
        assert.equal(borrowerOperationsRoles[3], false)

        const ownerRoles = await thusdTokenTester.getRoles(owner)
        assert.equal(ownerRoles[0], false)
        assert.equal(ownerRoles[1], false)
        assert.equal(ownerRoles[2], false)
        assert.equal(ownerRoles[3], false)
      })
    }
    ...
    if (!withProxy) {
      ...
      context("new set of system contracts is ready", () => {
        ...
        it('startAddContracts(): puts new set of contracts to pending list', async () => {
          ...
          assert.equal(troveManagerRoles[2], false)
          assert.equal(troveManagerRoles[3], true)
          const newTroveManagerRoles = await thusdTokenTester.getRoles(newTroveManager.address)
          assert.equal(newTroveManagerRoles[0], false)
          assert.equal(newTroveManagerRoles[1], false)
          assert.equal(newTroveManagerRoles[2], false)
          assert.equal(newTroveManagerRoles[3], false)
          ...
          assert.equal(stabilityPoolRoles[2], true)
          assert.equal(stabilityPoolRoles[3], true)
          const newStabilityPoolRoles = await thusdTokenTester.getRoles(newStabilityPool.address)
          assert.equal(newStabilityPoolRoles[0], false)
          assert.equal(newStabilityPoolRoles[1], false)
          assert.equal(newStabilityPoolRoles[2], false)
          assert.equal(newStabilityPoolRoles[3], false)
          ...
          assert.equal(borrowerOperationsRoles[2], false)
          assert.equal(borrowerOperationsRoles[3], false)
          const newBorrowerOperationsRoles = await thusdTokenTester.getRoles(newBorrowerOperations.address)
          assert.equal(newBorrowerOperationsRoles[0], false)
          assert.equal(newBorrowerOperationsRoles[1], false)
          assert.equal(newBorrowerOperationsRoles[2], false)
          assert.equal(newBorrowerOperationsRoles[3], false)
        })
        ...
        it('finalizeAddContracts(): enables new system contracts roles', async () => {
          ...
          const newTroveManagerRoles = await thusdTokenTester.getRoles(newTroveManager.address)
          assert.equal(newTroveManagerRoles[0], false)
          assert.equal(newTroveManagerRoles[1], true)
          assert.equal(newTroveManagerRoles[2], false)
          assert.equal(newTroveManagerRoles[3], true)
          ...
          const newStabilityPoolRoles = await thusdTokenTester.getRoles(newStabilityPool.address)
          assert.equal(newStabilityPoolRoles[0], false)
          assert.equal(newStabilityPoolRoles[1], true)
          assert.equal(newStabilityPoolRoles[2], true)
          assert.equal(newStabilityPoolRoles[3], true)
          ...
          assert.equal(borrowerOperationsRoles[3], false)
          const newBorrowerOperationsRoles = await thusdTokenTester.getRoles(newBorrowerOperations.address)
          assert.equal(newBorrowerOperationsRoles[0], true)
          assert.equal(newBorrowerOperationsRoles[1], true)
          assert.equal(newBorrowerOperationsRoles[2], false)
          assert.equal(newBorrowerOperationsRoles[3], false)
